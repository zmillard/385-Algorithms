/*******************************************************************************
 * Name        : rbtree.h
 * Author      : John Spicer, Anthony Picone
 * Version     : 1.0
 * Date        : 11/22/15
...
private:
	...
	void left_rotate(Node<K, V> *x) {
		...
		x->set_right(y->left());
		y->set_left(x->right());
		//static_cast<RedBlackNode<K, V>*>(y);
		if(y->left() != NULL){
			y->left()->set_parent(x);
		}
	...
	void right_rotate(Node<K, V> *x) {
		// TODO
		//this is left rotate, make it right
		Node<K, V> *y;
		y = x->left();
		x->set_left(y->right());
		y->set_right(x->left());
	...
	size_t leaf_count(Node<K, V> *node) const {
		...
		if(node->left() == NULL && node->right()==NULL){
			...
			leaves = leaf_count(node->left())+leaf_count(node->right());
		}
    ...
    int diameter(Node<K, V> *node) const {
		...
		if (node == NULL)
			return 0;
		int sum_height = (height(node->left()) + height(node->right()) );
		int max_diam = (diameter(node->left()) >= diameter(node->right())) ? diameter(node->left()) : diameter(node->right());

		return (sum_height >= max_diam) ? sum_height : max_diam;
    }
    ...
    size_t null_count(Node<K, V> *node) const {
		...
		if (node == NULL)
			return 1;
		if (node->left() != NULL)
			nulls += null_count(node->left());
		else
			nulls++;
		if (node->right() != NULL)
			nulls += null_count(node->right());
		else
			nulls++;
		return nulls;
	}
    ...
    /**
        ...
        size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        	...
        	if(node != NULL){
    			...
    			int s1 = sum_null_levels(node->left(), ++level);
    			int s2 = sum_null_levels(node->right(), ++level);
    			return s1 + s2;
    		}*/
    		return level;
        }
