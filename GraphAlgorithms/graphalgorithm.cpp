...
static void depth_first_search_visit(vertex_id vertex, const Graph * const graph, bool *visited, vector<vertex_id> &order){
	visited[vertex]= true;
	order.push_back(vertex);
	vector<vertex_id> adjacent = graph->get_adjacent(vertex);
	for(vector<vertex_id>::iterator it = adjacent.begin(); it != adjacent.end(); ++it){ //way to iterate through vertex
		if(!visited[*it]){//check if it is marked as 0;
			depth_first_search_visit(*it, graph, visited, order);
		}
	}
}
vector<vertex_id> top_sort(const Graph * const graph) {
	vector<vertex_id> order, vertices = graph->get_vertices();
	size_t num_vertices = static_cast<size_t>(graph->get_highest_vertex_id());
	order.reserve(num_vertices); //allocates memory
